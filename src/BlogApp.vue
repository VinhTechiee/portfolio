...
const routeLinks = ["blog", "resume"];
const sections = ["home", "projects", "blog", "resume", "contact"];
...
const handleLinkClick = (link: string) => {
  if (routeLinks.includes(link)) {
    window.location.href = `${baseUrl}blog/#/${link === "blog" ? "" : link}`;
    return;
  }

  window.location.href = link === "home" ? baseUrl : `${baseUrl}#${link}`;
};
</script>
